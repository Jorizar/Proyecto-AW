...
class FormularioCambioAdmin extends Formulario
{
    protected $id = " ";

    public function __construct($idu)
    {
        parent::__construct('formCambioDatos', ['urlRedireccion' => Aplicacion::getInstance()->resuelve('/admin_usuarios.php'), 'enctype' => 'multipart/form-data']);
        $this->id = $idu;
    }
    ...
    protected function generaCamposFormulario(&$datos)
    {
        ...
        if (!empty($this->id)) {
            $htmlUsuarioId = "<input type='hidden' name='usuario_id' value='{$this->id}'>";
        }
    ...
    protected function procesaFormulario(&$datos)
    {
        ...
        if (isset($datos['nuevo_nombre']) && !empty(trim($datos['nuevo_nombre']))) {
            ...
            if (!isset($this->errores['nuevo_nombre'])) {
                ...
                if ($user) {
                    ...
                    $result = Usuario::cambiarNombre($this->id, $nuevoNombre);
                }
            }
        }
        ...
        if (!empty($nuevoEmail)) {
            ...
            if (!filter_var($nuevoEmail, FILTER_VALIDATE_EMAIL)) {
                ...
                $result = Usuario::cambiarEmail($this->id, $nuevoEmail); 
            }
        }
        ...
        if (isset($_FILES['nueva_foto'])) {
            ...
            if (in_array($filetype, $allowTypes)) { //Comprobamos que la extensión de la imagen se ajusta a las requeridas
                if (move_uploaded_file($_FILES['nueva_foto']["tmp_name"], $targetFilePath)) {
                    ...
                    $result = Usuario::actualizaFoto($this->id, $targetFilePath);
